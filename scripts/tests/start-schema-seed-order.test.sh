...
schema_line="$(grep -n "mysql:schema-import" "$LOG_FILE" | cut -d: -f1 || true)"
seed_line="$(grep -n "mysql:seed-import" "$LOG_FILE" | cut -d: -f1 || true)"

if [[ -z "$schema_line" ]]; then
  echo "Expected scripts/start.sh to import backend/src/main/resources/schema.sql before seed data." >&2
...
if [[ -z "$seed_line" ]]; then
  echo "Expected scripts/start.sh to import scripts/seed-data.sql after schema." >&2
  echo "Observed calls:" >&2
  cat "$LOG_FILE" >&2
  exit 1
fi

if (( schema_line >= seed_line )); then
  echo "Expected schema import before seed import." >&2
  echo "Observed calls:" >&2
  cat "$LOG_FILE" >&2
  exit 1
fi
