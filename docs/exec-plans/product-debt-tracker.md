...
| PD-009 | Outbound FIFO | Week4 仅实现“普通按单出库时阻断非最早剩余看板”，但对跨单补锁、人工改配后是否允许提示后继续、以及无单强制出库是否仍显示 FIFO 提示尚未定稿 | 确认 FIFO 在普通出库、强制出库、改配/补锁场景下是“硬阻断”还是“提示并允许继续”，以及前端是否需要例外授权流程 | open |
