...
- 将原“看板”领域对象统一迁移为“库存标签”，并将库存标签码前缀从 `KB:v1:` 全量改为 `IT:v1:`。
...
库存标签是具体库存载体标识，原“看板/看板码”迁移而来。库存标签描述一箱、一板或一批库存的标签码、所属入库单明细、数量、库位、生命周期和占用状态。
...
本项目当前没有生产数据，本分支采用破坏式 schema 重建，不提供历史兼容迁移脚本。

迁移要求：

- `kanban_board` 表重命名为 `inventory_tag`。
- `kanban_code` 字段重命名为 `inventory_tag_code`。
- 所有外键、索引、mapper、实体、DTO、测试数据和种子数据同步使用库存标签命名。
- `data.sql` 中的旧 `KB:v1:` 码值全量改为 `IT:v1:`。
...
- `kanbans` -> `inventoryTags`

如路由路径仍保留 `/kanbans`，必须在本分支内同步迁移为库存标签语义路径，避免 API 与领域术语不一致。
