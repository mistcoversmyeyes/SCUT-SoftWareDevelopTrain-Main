...
- 将原“看板/看板单/kanban_board”领域对象统一迁移为“库存标签”，并将库存标签码前缀从 `KB:v1:` 全量改为 `IT:v1:`。
...
库存标签是具体库存载体标识，由原“看板/看板单/kanban_board（看板码）”迁移而来。库存标签描述一箱、一板或一批库存的标签码、所属入库单明细、数量、库位、生命周期和占用状态。
