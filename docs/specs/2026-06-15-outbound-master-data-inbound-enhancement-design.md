...
本设计服务于 Week 3 周迭代。当前代码基线为 Week 2 产出：入库单 CRUD、库存标签生成/打印、扫码入库、库存余额/追溯、库存标签追溯。本周在此基础上完成出库闭环、基础数据 CRUD、入库模块增强和库存标签生命周期完善。后续 Week 4 已将“库存标签”作为库存载体对象的正式命名，本文按当前实现口径同步旧称。
...
- 出库拣货：带单出库、不带单出库，库存标签码扫描，FIFO 先进先出，扣减库存，更新库存标签状态
...
- 查看/打印库存标签（替代原来的"打印库存标签"按钮）：标签页库存标签列表 + 二维码 + 打印
- 入库历史：按时间范围查询已完成/已取消的入库单
- 库存标签列表：多维度筛选，查看库存标签生命周期状态
...
-- inventory_tag: 新增已出库数量
ALTER TABLE inventory_tag
  ADD COLUMN picked_qty DECIMAL(18, 3) NOT NULL DEFAULT 0 AFTER board_qty;
```

### 枚举扩展

- `inventory_tag.status`：增加 `SHIPPED`（已出库）
...
| `com.scut.wms.outbound.picking` | 出库拣货（FIFO） |
...
| `GET` | `/api/inventory/inventory-tags?status=&inboundNo=&materialCode=` | 库存标签列表多维度查询 |
| `GET` | `/api/inbound-orders/{id}/inventory-tags` | 获取入库单对应的库存标签列表 |
...
| `POST` | `/api/outbound-orders/{id}/release-and-lock` | 释放出库单并自动锁货 |
| `POST` | `/api/outbound-orders/{id}/cancel` | 取消出库单 |
| `GET` | `/api/outbound-orders/{id}/print` | 获取出库单打印数据 |
| `GET` | `/api/outbound/inventory-tag-lookup` | 出库前按库存标签码查询库存标签上下文 |
| `POST` | `/api/outbound/pick-with-order` | 带单扫描库存标签码执行 FIFO 出库 |
| `POST` | `/api/outbound/pick-with-order/force` | 带单强制扫描库存标签码出库 |
| `POST` | `/api/outbound/pick-no-order` | 不带单扫描库存标签码出库 |
...
`POST /api/outbound/pick-with-order`、`POST /api/outbound/pick-no-order` 事务边界：

1. 按 `inventoryTagCode` 查询库存标签并 `FOR UPDATE` 加锁
2. 库存标签不存在 → 返回错误
3. 库存标签状态不是 `RECEIVED` → 返回"库存标签不可出库"
4. FIFO 校验：带单出库查询该物料在目标库位是否有更早入库的库存标签未出库，如有则提示"请优先出库更早批次"；强制出库路径记录强制操作
5. 写入 `inventory_movement`：`movement_type = OUTBOUND_PICK`
6. 扣减 `inventory_balance.on_hand_qty`
7. 累加 `inventory_tag.picked_qty`，若 `picked_qty >= board_qty` 则状态改为 `SHIPPED`
8. 提交事务
...
🏷️ 库存标签信息
   ├ 库存标签列表      /inventory-tags/list
   └ 库存标签追溯      /inventory-tags/trace
```
...
| 2 | `InventoryTagDetailView` | `/inbound/:id/inventory-tags` | **改造**：替代打印弹窗，库存标签列表+二维码+打印 |
| 3 | `InboundOrderListView` | `/inbound/orders` | **小改**：按钮文字"打印入库单"→"查看/打印入库单"，"打印库存标签"→"查看/打印库存标签" |
| 4 | `InboundHistoryView` | `/inbound/history` | **新增**：入库历史 |
| 5 | `InventoryTagListView` | `/inventory-tags/list` | **新增**：库存标签列表多维度筛选 |
| 6 | `OutboundOrderListView` | `/outbound/orders` | **新增**：出库单管理 |
| 7 | `OutboundPickWithOrderView` | `/outbound/pick-with-order` | **新增**：带单出库 |
| 8 | `OutboundPickNoOrderView` | `/outbound/pick-no-order` | **新增**：不带单出库 |
| 9 | `OutboundLockView` | `/outbound/locks` | **新增**：锁货管理 |
| 10 | `OutboundHistoryView` | `/outbound/history` | **新增**：出库历史 |
| 11 | `DashboardView` | `/dashboard` | **改造**：统计卡片+预警列表 |
| 12 | `SupplierListView` | `/master-data/suppliers` | **新增**：供应商管理 |
| 13 | `MaterialListView` | `/master-data/materials` | **新增**：物料管理 |
| 14 | `ContainerListView` | `/master-data/containers` | **新增**：器具管理 |
| 15 | `WarehouseLocationView` | `/master-data/warehouses` | **新增**：仓库库位管理 |
...
**库存标签详情/打印页 (`InventoryTagDetailView`)：**
- 标签页模式，路由 `/inbound/:id/inventory-tags`
...
后端：
- 运行 `mvn test`
- 覆盖：出库单 CRUD、释放/取消、FIFO 出库拣货、库存扣减、库存标签状态更新、基础数据 CRUD
...
3. 入库单：创建→释放→查看/打印入库单→查看/打印库存标签（验证二维码）
4. 入库扫码→查看库存→库存标签列表验证生命周期
5. 出库单：创建→释放并锁货
6. 带单出库/不带单出库→FIFO 验证→库存扣减→库存标签状态变为 SHIPPED
7. 入库历史/出库历史查看
8. Dashboard 首页统计+预警
