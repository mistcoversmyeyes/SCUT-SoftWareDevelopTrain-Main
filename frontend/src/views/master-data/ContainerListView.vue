<template>
  <section class="container-page">
    <el-card>
      ...
      <el-table v-loading="loading" :data="containerTypes" border stripe size="small" class="data-table">
...
const form = reactive({
  containerCode: '', containerName: '', capacityQty: 0, status: 'ACTIVE'
})
...
async function loadContainerTypes() {
  ...
  try { containerTypes.value = await fetchContainerTypes() }
  catch (error) { loadError.value = error.response?.data?.message || '容器类型列表加载失败'; containerTypes.value = [] }
  finally { loading.value = false }
}

function resetForm() { form.containerCode = ''; form.containerName = ''; form.capacityQty = 0; form.status = 'ACTIVE' }
...
function openEditDrawer(row) {
  ...
  form.containerCode = row.containerCode; form.containerName = row.containerName
...
async function handleSave() {
  ...
  saving.value = true
  try {
    const payload = { containerCode: form.containerCode, containerName: form.containerName, capacityQty: form.capacityQty, status: form.status }
    if (drawerMode.value === 'edit') { await updateContainerType(editingId.value, payload); ElMessage.success('容器类型修改成功') }
    else { await createContainerType(payload); ElMessage.success('容器类型创建成功') }
    drawerVisible.value = false
    await loadContainerTypes()
  } catch (error) { ElMessage.error(error.response?.data?.message || '保存失败，请重试') }
...
async function handleStatusChange(row, val) {
  ...
  try {
    await updateContainerTypeStatus(row.id, val ? 'ACTIVE' : 'INACTIVE')
    ElMessage.success(val ? '容器类型已启用' : '容器类型已停用')
    await loadContainerTypes()
  } catch (error) { ElMessage.error(error.response?.data?.message || '状态更新失败') }
  finally { row._statusLoading = false }
}

onMounted(() => { loadContainerTypes() })
</script>

<style scoped>
.container-page :deep(.el-card__body) { padding-top: 12px; }
